...
import select
import datetime
from glider_msg.msg import altimsg
...
class Socket(object):
    ...
    def get_reply(self, expected_reply = None):
        ...
        try:
            ...
            rospy.logdebug("Failed to get Reply")
    ...
    *VA500* class for sonar altimeter
    ...
    def scan(self):
        ...
        while not self.preempted:
            ...
            altiPublisher = rospy.Publisher('messageAlti',altimsg, queue_size=10)
            msg = altimsg()
            msg.range = self.range
            msg.depth = self.depth 
            msg.minRange = self.min_range
            msg.maxRange = self.max_range
            try:
                altiPublisher.publish(msg)
            except:
                pass
...
if __name__ == "__main__":
    # Initialize node
    rospy.init_node('node_Alti', log_level=rospy.DEBUG)
