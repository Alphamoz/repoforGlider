...
int fd,fd1, fd2;
char *portname1 = "/dev/ttyxuart0";
char *portname2 = "/dev/ttyxuart1";
int wlen1, wlen2;
...
void command_line() {
	...
	if (mystr.find(key1) != std::string::npos) 
		{
			cout << "ack AT"<<endl; 	
		} 
	else
	if (mystr.find(key2) != std::string::npos) 
		{
			cout << "ack RTB"<<endl; 	
		} 
	else
	if (mystr.find(key3) != std::string::npos) 
		{
			cout << "ack START"<<endl; 	
		} 
	else
	if (mystr.find(key4) != std::string::npos)
		{
			mystr = mystr.erase (0,5);
			cout << "ack "<<mystr<< endl;
		}
	else
	if (mystr.find(key5) != std::string::npos)
		{
			mystr = mystr.erase (0,5);
			cout << "ack "<<mystr<< endl;
		}
