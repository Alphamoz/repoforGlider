...
int main(){
	int sport = open("/dev/ttyxuart0", O_RDWR | O_NOCTTY | O_SYNC);
	cout << "port open : " <<sport<< endl;
	if(sport < 0) {
		printf("Error opening %s: %s\n", "/dev/ttyxuart0", strerror(errno));
		return -1;
	}
...
setattribs();
	...
	SerWrite(sport, "data sent through COM-0\r\n");
	SerWrite(sport, "ttyxuart0 - baudrate 9600\r\n");
	while(1){
	SerWrite(sport, "data sent through COM-0 \r\n");
