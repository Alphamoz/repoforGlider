...
string thread_sensor_id;
string thread_control_id;
string thread_fsm_id;
...
float surface_depth = 0, depth_threshold = 2.0, depth_target = 2.8;
...
void handlingPID() {
    ...
    printf("stream_sensor : %d, stream_control : %d\n", stream_sensor, stream_control);
    if(stream_control == true) {
        ...
        else if(global_depth >= depth_threshold) {
            ascent_flag = 1;
        }
...
void glideOperation() {
    if(ascent_flag == true) {
        ...
        while(global_depth >= depth_threshold || global_depth == depth_target) {
            printf("Start Ascent\n");
            ascentValue();
...
void descentValue() {
    ...
    pthread_mutex_lock(&lock_flag);
    printf("Mutex locked on descent\n");
    ref_pitch = ref_pitch_d;
    ref_depth = ref_depth_d;
    sleep(1);
    pthread_mutex_unlock(&lock_flag);
    printf("Mutex unlocked on descent\n");
...
void ascentValue() {
    ...
    pthread_mutex_lock(&lock_flag);
    printf("Mutex locked on ascent\n");
    ref_pitch = ref_pitch_a;
    ref_depth = ref_depth_a;
    sleep(1);
    pthread_mutex_unlock(&lock_flag);
    printf("Mutex unlocked on ascent\n");
...
void commandLine() {
    ...
    if(mystr == "#GB> atc operation") {
        ...
        cout << "Check FSM thread, ID : " << thread_fsm_id << endl;
        cout << "Check Control thread, ID : " << thread_control_id << endl;
        cout << "Check Sensor thread, ID : " << thread_sensor_id << endl;
    }
