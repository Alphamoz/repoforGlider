...
std::string GCS_MANUALGLIDE = "#GB> MANUALGLIDE";
...
int ParsingGCS(std::string str,bool _debug){
	...
	if (str.find(GCS_SELFTEST) != std::string::npos)
	{
		if (_debug)
		{
			std::cout<<"debug : ack SELFTEST"<< std::endl;
		}
		_mode = SELFTEST;
	}else
	if (str.find(GCS_MANUALGLIDE) != std::string::npos)
	{
		if (_debug) 
		{
			std::string _str = str.erase (0,5);
	...
	if (str.find(GCS_PITCHCONTROL) != std::string::npos)
	{
		if (_debug)
		{
	...
	if(str.find(GCS_DEPTHCONTROL) != std::string::npos)
	{
		if (_debug)
                {
