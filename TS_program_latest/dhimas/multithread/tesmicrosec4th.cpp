...
int dur_pid_bladder;
int dur_pid_ballast;
int dur_sensor_parse;
int dur_fsm_finished;
...
void PID_rd() {
	auto start_pid_rudder = high_resolution_clock::now();
	...
	auto stop_pid_rudder = high_resolution_clock()::now();

	auto duration = duration_cast<microseconds>(stop_pid_rudder - start_pid_rudder);
	dur_pid_rudder = duration.count();
}

void PID_blst() {
	auto start_pid_ballast = high_resolution_clock()::now();
	...
	auto stop_pid_ballast = high_resolution_clock()::now();

	auto duration = duration_cast<microseconds>(stop_pid_ballast - start_pid_ballast);
	dur_pid_ballast = duration.count();
}

void PID_bld() {
	auto start_pid_bladder = high_resolution_clock()::now();
	...
	auto stop_pid_bladder = high_resolution_clock()::now();

	auto duration = duration_cast<microseconds>(stop_pid_bladder - start_pid_bladder);
	dur_pid_bladder = duration.count();
}

void handlingBB(int _fd) {
	while(stream_sensor == true) {
		auto start_sensor_parse = high_resolution_clock()::now();
		...
		sleep(2);   
	    auto stop_sensor_parse = high_resolution_clock()::now();

	    auto duration = duration_cast<microseconds>(stop_sensor_parse - start_sensor_parse);
	    dur_sensor_parse = duration.count();

	    ofstream logparsingBB("state_function_thread.txt", ios::out | ios::app);
	    if(logparsingBB.is_open()) {
	        logparsingBB << buftimeBB << " | " << "Parsing Finished, Duration : " << dur_sensor_parse << endl;
	        logparsingBB.close();
	    }
...
void handlingPID() {
	...
	if(stream_control == true) {
		...
		handler_fsm = 1;
		PID_rd();
		cout << "PID rudder : " << rd_PID_val << ", Duration : " << dur_pid_rudder << endl;
		PID_bld();
		cout << "PID bladder : " << bld_PID_val << ", Duration : " << dur_pid_bladder << endl;
		PID_blst();
		cout << "PID ballast : " << blst_PID_val << ", Duration : " << dur_pid_ballast << endl;

		ofstream controlvaluelog("value_control.txt", ios ::out | ios::app);
		if(controlvaluelog.is_open()) {
			controlvaluelog << control_tm_info << "| Error rudder : " << heading_err << ", PID rudder : " << rd_PID_val << ", Duration : " << dur_pid_rudder << endl;
			controlvaluelog << control_tm_info << "| Error bladder : " << pitch_err << ", PID bladder : " << bld_PID_val << ", Duration : " << dur_pid_bladder << endl;
			controlvaluelog << control_tm_info << "| Error ballast : " << depth_err << ", PID ballast : " << blst_PID_val << ", Duration : " << dur_pid_ballast << endl;
...
void handlingFSM() {
	...
	switch(mode) {
		case 1 : //operation
			{
				auto start_fsm_time = high_resolution_clock()::now();
				...
				auto stop_fsm_time = high_resolution_clock()::now();

				auto duration = duration_cast<microseconds>(stop_fsm_time - start_fsm_time);
				dur_fsm_finished = duration.count();
				ofstream opstate("log_state.txt", ios ::out | ios::app);
				if(opstate.is_open()) {
					opstate << fsm_tm_info << ' ' << "State : Stop Operation, Duration : " << dur_fsm_finished << endl;
					opstate.close();
				}
				sleep(1);
			}
		...
		case 2 : //gliding
			{
				...
				ofstream glstate("log_state.txt", ios ::out | ios::app);
				if(glstate.is_open()) {
					glstate << fsm_tm_info << ' ' << "State : Stop Gliding, Duration : " << dur_fsm_finished << endl;
					glstate.close();
				}
				sleep(1);
			}
		...
		case 3 : //propeller test
			{
				...
				auto duration = duration_cast<microseconds>(stop_fsm_time - start_fsm_time);
				dur_fsm_finished = duration.count();
				ofstream propstate("log_state.txt", ios ::out | ios::app);
				if(propstate.is_open()) {
					propstate << fsm_tm_info << ' ' << "State : Stop Propeller Test, Duration : " << dur_fsm_finished << endl;
					propstate.close();
				}
			}
			break;
		...
		case 4 : //bowthruster test
			{
				...
				auto duration = duration_cast<microseconds>(stop_fsm_time - start_fsm_time);
				dur_fsm_finished = duration.count();
				ofstream bowstate("log_state.txt", ios ::out | ios::app);
				if(bowstate.is_open()) {
					bowstate << fsm_tm_info << ' ' << "State : Stop Bowthrusters Test, Duration : " << dur_fsm_finished << endl;
					bowstate.close();
				}
			}
			break;
		...
		case 5 : //manual op
			{
				...
				auto duration = duration_cast<microseconds>(stop_fsm_time - start_fsm_time);
				dur_fsm_finished = duration.count();
				ofstream datalogstate("log_state.txt", ios ::out | ios::app);
				if(datalogstate.is_open()) {
					datalogstate << fsm_tm_info << ' ' << "State : Stop Broadcast Datalog, Duration : " << dur_fsm_finished << endl;
					datalogstate.close();
				}
			}
			break;
