...
double sum_IMU_y = 0;
double sum_IMU_z = 0;
double now_sum_IMU_x = 0;
double now_sum_IMU_y = 0;
double now_sum_IMU_z = 0;
// ------------------------------------------------------------------------
...
//Navigation calculate_Nav(Navigation _nav, sensor Sensor);
...
int main(){
    ...
    int fd = open("/sys/class/gpio/gpio46/value", O_RDWR | O_CLOEXEC ); // Relay$
    if( fd < 0 ) {
...
// Serial Setting ---------------------------------------------------------
    ...
    if(Flag.BeagleMatlab == 1){
        ...
        if(!USB.isSet()) cout << USB.loc << "Open Error" << endl;
    }
		...
		//usleep(200000);
                if(Flag.State > 0 || Flag.BeagleActive){
                    ...
                    if(Flag.IMUActive){
                        ...
                        if (smoot_IMU == 10){
			    now_sum_IMU_x = sum_IMU_x/10;
			    now_sum_IMU_y = sum_IMU_y/10;
			    now_sum_IMU_z = sum_IMU_z/10;
			    sum_IMU_x = 0;
			    sum_IMU_y = 0;
			    sum_IMU_z = 0;
			    smoot_IMU = 0;
			    ...
			    smoot_IMU = smoot_IMU + 1;
			    sum_IMU_x = sum_IMU_x + Sensors.valIMU.accel_X;
			    sum_IMU_y = sum_IMU_y + Sensors.valIMU.accel_Y;
			    sum_IMU_z = sum_IMU_z + Sensors.valIMU.accel_Z;
			}

		        printf("Reading IMU\n");
                        printf("\tLatitude = %f, Longitude = %f, Height = %f\n", Sensors.valIMU.Latitude, Sensors.valIMU.Longitude, Sensors.valIMU.Height);
                        printf("\tAccelerometers X: %f Y: %f Z: %f\n", Sensors.valIMU.accel_X, Sensors.valIMU.accel_Y, Sensors.valIMU.accel_Z);
                        printf("\tSudut Roll: %f Pitch: %f Heading: %f\n", Sensors.valIMU.Roll, Sensors.valIMU.Pitch, Sensors.valIMU.Heading);
                        printf("\tGyroscope Roll: %f Pitch: %f Heading: %f\n", Sensors.valIMU.Omega_Roll, Sensors.valIMU.Omega_Pitch, Sensors.valIMU.Omega_Heading);
			//Navigation=calculate_Nav(Navigation);
			calculate_Nav();
			//usleep(250000);
...
//			usleep(1000000);
				...
				dataSend.push_back(Sensors.valIMU.Pitch);
				dataSend.push_back(Sensors.valIMU.Heading);
				dataSend.push_back(Sensors.valIMU.Roll);
                                dataSend.push_back(now_sum_IMU_x);
				dataSend.push_back(now_sum_IMU_y);
				dataSend.push_back(now_sum_IMU_z);
				...
				printf("\tvDVL  X: %f Y: %f Z: %f\n", Sensors.valDVL.veloX, Sensors.valDVL.veloY, Sensors.valDVL.veloZ);

				cout << "Send Data Sensor to TS " << endl;
...
void kalman_imu(){
   ...
   double nAcc;
   double nMag;
   static const double dv4[7] = { 1.0E-6, 1.0E-6, 1.0E-6, 1.0E-6, 1.0E-8, 1.0E-8, 1.0E-8 };
   static const double dv5[7] = { nGyro * nGyro, nGyro * nGyro, nGyro * nGyro, nGyro * nGyro, 1.0E-9, 1.0E-9, 1.0E-9 };
   static const double dv6[6] = { nAcc * nAcc, nAcc * nAcc, nAcc * nAcc, nMag, nMag, nMag };
   g = 9.81;
   ...
   double p = Sensors.valIMU.Omega_Roll;
   double q = Sensors.valIMU.Omega_Pitch;
   double r = Sensors.valIMU.Omega_Heading;
   double ax = Sensors.valIMU.accel_X;
   double ay = Sensors.valIMU.accel_Y;
   double az = Sensors.valIMU.accel_Z;
   double mx = Sensors.valIMU.Roll*toRad;
   double my = Sensors.valIMU.Pitch*toRad;
   double mz = Sensors.valIMU.Heading*toRad;
   double Va = 0.0;
   double gyr_rps[3];
   double acc_mps2[3];
   double mag_unit[3];
...
void calculate_Nav(){  
 //Navigation nav = _nav;
   ...
   double toRad = PI/180;
   double g = 9.8335;
   double Omega_Pitch_rad = Sensors.valIMU.Pitch*toRad;//dalam bentuk radian
   double Omega_Heading_rad = Sensors.valIMU.Heading*toRad;//dalam bentuk radian
   double Omega_Roll_rad = Sensors.valIMU.Roll*toRad;//dalam bentuk radian

   //Transformasi koordinat IMU
   //xdot_ned adalah akselesrasi dibidang bilang earth 
   double xdot_ned = now_sum_IMU_x*(cos(Omega_Pitch_rad)*cos(Omega_Heading_rad))+
                     now_sum_IMU_y*((sin(Omega_Roll_rad)*sin(Omega_Pitch_rad)*cos(Omega_Heading_rad))-(cos(Omega_Roll_rad)*sin(Omega_Heading_rad)))+
                     now_sum_IMU_z*((cos(Omega_Roll_rad)*sin(Omega_Pitch_rad)*cos(Omega_Heading_rad))+(sin(Omega_Roll_rad)*sin(Omega_Heading_rad)));

   double ydot_ned = now_sum_IMU_x*(cos(Omega_Pitch_rad)*sin(Omega_Heading_rad))+
                     now_sum_IMU_y*((sin(Omega_Roll_rad)*sin(Omega_Pitch_rad)*sin(Omega_Heading_rad))+(cos(Omega_Roll_rad)*cos(Omega_Heading_rad)))+
                     now_sum_IMU_z*((cos(Omega_Roll_rad)*sin(Omega_Pitch_rad)*sin(Omega_Heading_rad))-(sin(Omega_Roll_rad)*cos(Omega_Heading_rad)));

   double zdot_ned = now_sum_IMU_x*(-1*sin(Omega_Pitch_rad))+
                     now_sum_IMU_y*(sin(Omega_Roll_rad)*cos(Omega_Pitch_rad))+
                     now_sum_IMU_z*(cos(Omega_Roll_rad)*cos(Omega_Pitch_rad))+g;


   printf("\n");
   //printf("\tCalculate_Nav  udot_ned: %f vdot_ned: %f wdot_net: %f\n", udot_ned, vdot_ned, wdot_ned);

   if (latitude==0)
       latitude = Sensors.valIMU.Latitude;
   else 
  ...
  // double vD_IMU;
   ...
   double aIMU_N = xdot_ned;//-2*omegaE*sin(latitude)*vE_IMU-(vE_IMU*vE_IMU)/R*tan(latitude)+vN_IMU*vD_IMU/R;
   double aIMU_E = ydot_ned;//-2*omegaE*sin(latitude)*vN_IMU-vE_IMU*vN_IMU/R*tan(latitude)-2*omegaE*cos(latitude)*vD_IMU-vE_IMU*vD_IMU/R;
   double aIMU_D = zdot_ned;//-2*omegaE*cos(latitude)*vE_IMU-(vE_IMU*vE_IMU)/R-(vN_IMU*vN_IMU)/R;
   
   printf("\taIMU  N: %f E: %f D: %f\n", aIMU_N, aIMU_E, aIMU_D);

   //double aIMU_N = udot_ned-2*omegaE*sin(latitude)*vE_IMU-(vE_IMU*vE_IMU)/R*tan(latitude)+vN_IMU*vD_IMU/R;
   //double aIMU_E = vdot_ned-2*omegaE*sin(latitude)*vN_IMU-vE_IMU*vN_IMU/R*tan(latitude)-2*omegaE*cos(latitude)*vD_IMU-vE_IMU*vD_IMU/R;
   //double aIMU_D = wdot_ned-2*omegaE*cos(latitude)*vE_IMU-(vE_IMU*vE_IMU)/R-(vN_IMU*vN_IMU)/R;
   ...
   vN_IMU = vN_IMU + aIMU_N * 0.25;//kali selang waktu
   vE_IMU = vE_IMU + aIMU_E * 0.25;//kali selang waktu
   vD_IMU = vD_IMU + aIMU_D * 0.25;//kali selang waktu
   printf("\n");
   printf("\tVIMU_NED  vN_IMU: %f vE_IMU: %f vD_IMU: %f\n", vN_IMU, vE_IMU, vD_IMU);
   //return nav;
   //LogData();
}
...
void ReadSetting(char* configtxt){
    ...
    while(getline(data,str)){
        ...
        if (str.find(Str_BeagleMatlab) != std::string::npos){
            str = str.erase(0,str.length()-2);
            str = str.erase(1);
...
void ReadCommand(int _fd, string command){
    ...
    if (command.find(TS_BB_OPERATE) != std::string::npos){
        ...
        if(command.substr(0,1) == "2"){
            SerWrite(_fd,"#BB> ack OPERATION\r\n");
            printf("Standby state\r\n");
            Flag.State = 2;
            Flag.Sampling = 1;
        } else
        if(command.substr(0,1) == "3"){
            SerWrite(_fd,"#BB> ack OPERATION\r\n");
            printf("Read Sensor state\r\n");
            Flag.State = 3;
            Flag.Sampling = 1;
        } else
        if(command.substr(0,1) == "4"){
            SerWrite(_fd,"#BB> ack OPERATION\r\n");
...
void LogData(){
    ...
    //fileName << "BB_LOG_Sekarang" << Times.DateTime[0] << "-" << Times.DateTime[1] << "-"  << Times.DateTime[2] << ".txt";
    fileName << "BB_LOG_Sekarang1111" << "-" << "-" << ".txt";
    ...
    if (Log.newFile=1)
    {
        ...
        sprintf(datatulis,"accX, accY, accZ, filaccX, filaccY, filaccZ, Roll, Pitch, Heading, aIMU_N, aIMU_E, aIMU_D, longitude, latitude, vDVL_X, vDVL_Y, vDVL_Z  \n");
...
//    now_sum_IMU_x
    sprintf(datatulis, "%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f",
    Sensors.valIMU.accel_X, Sensors.valIMU.accel_Y, Sensors.valIMU.accel_Z, now_sum_IMU_x, now_sum_IMU_y, now_sum_IMU_z,
    Sensors.valIMU.Roll, Sensors.valIMU.Pitch,
    Sensors.valIMU.Heading, aIMU_N, aIMU_E, aIMU_D,Sensors.valIMU.Longitude, Sensors.valIMU.Latitude, Sensors.valDVL.veloX,
    Sensors.valDVL.veloY, Sensors.valDVL.veloZ);


//    sprintf(datatulis, "%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f,%5f",
//    Sensors.valIMU.accel_X, Sensors.valIMU.accel_Y, Sensors.valIMU.accel_Z, Sensors.valIMU.Roll, Sensors.valIMU.Pitch,
//    Sensors.valIMU.Heading, aIMU_N, aIMU_E, aIMU_D,Sensors.valIMU.Longitude, Sensors.valIMU.Latitude);
...
void ParseDVL(string Parse_DVL){
    ...
    while (pch != NULL)
    {
        ...
        case 23 : 
            // Velocity X
            strncpy(Sensors.buff, pch, Sensors.maxBuff);
            Sensors.valDVL.veloX = atof (Sensors.buff);
            datacount++;
            break;
        case 24 : 
            // Velocity Y
            strncpy(Sensors.buff, pch, Sensors.maxBuff);
            Sensors.valDVL.veloY = atof (Sensors.buff);
            datacount++;
            break;
        case 25 : 
            // Velocity Z
            strncpy(Sensors.buff, pch, Sensors.maxBuff);
            Sensors.valDVL.veloZ = atof (Sensors.buff);
            datacount++;
            break;
        case 26 : 
            // Velocity Flag
