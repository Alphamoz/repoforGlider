...
// Program Setting Definition =============================================
    ...
    int DummyCoordinate; //Set 1 untuk mengambil kordinat dummy dari file DummyCoordinate.txt
...
// Config.txt setting var ================================================
    ...
    const string Str_DummyCoordinate = "DummyCoordinate";

// Com_Mainboard variables ================================================
    const string TS_BB_PING          = "#TS> PING";
...
// All function declared here! ============================================
    ...
    void FakeCoordinate(char* coordtxt, int line);
...
// Variabel untuk Dummy Coordinate =======================================
    int DC_i = 0;
    int DC_j = 0;
...
// Whlie loop here! ----------------------------------------------------
    ...
    while(1){
        ...
        /*
            PING == 1 baca semua sensor, kirim ackPING
            SensorRead == 1 baca semua sensor, kirim data
            ProcessRunning == 1 baca semua sensor, nav guidance, kirim data
            ...
            state BB:
                ...
                - 1 standby = baca priority sensor, kirim data
                - 2 readSensor = baca semua sensor, kirim data
                - 3 PINGed = baca semua sensor, kirim ackPING
                - 4 Operate = baca semua sensor, nav, guidance, kirim data
            
                //TS OPERATE 0 ==>> state = 0
                //TS OPERATE 1 ==>> state = 1
                //TS OPERATE 2 ==>> state = 2
                //TS PING / TS OPERATE 3 ==>> state = 3
                //TS OPERATE 4 ==>> state = 4
                
                //TS OPERATE 3 == TS PING
...
/*
                ParseDVL(//input,output)
                ParseDVL(DVL_Data,DVLResult)
                DVLResult.

                void ParseDVL(string data, DVL d)
                {
                    d.x = ....
                    d.y = .....                    
                }
*/
            ...
            if (ProcessRunning == 1){
                ...
                if(DummyCoordinate){
                    if (DC_i == 5 || DC_i == 0){
                        cout << "DC_j Increment" << DC_j << endl;
                        FakeCoordinate("DummyCoordinate.txt", DC_j);
                        DC_i = 0;
                        DC_j += 1;
                    }
                    DC_i += 1;
                    cout << "DC_i Increment" << DC_i << endl;
                }
                ...
                times = (finish.tv_sec - start.tv_sec);
                cout << "time needed : " << times << endl;
            ...
            GuidanceLOS();
            clock_gettime(CLOCK_MONOTONIC, &finish);
            times = (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
            cout << "time needed : " << times << endl;
        ...
        if (SR != 0 && (SensorRead == 1 || ProcessRunning == 1)){
            if(Sampling < (SR)){
                ...
                Sampling += times;
                // cout << "\nSampling time\t: " << SR << endl;
                // cout << "Elapsed time\t: " << Sampling << endl;
                // cout << "Time Left\t: " << ((float)SR)-Sampling << " sec" << endl;
            }
            else{
                FLag = 1;
                Sampling = 0;
...
void ReadSetting(char* configtxt){
    ...
    while(getline(data,str)){
        ...
        if (str.find(Str_DummyCoordinate) != std::string::npos){
            str = str.erase(0,str.length()-2);
            str = str.erase(1);
            printf("Dummy Coord.\t= %d\n", stoi(str));
            DummyCoordinate = stoi(str);
        }
...
void ParseWaypoint(string command){
    ...
    if (lat_start==lon_start==lat_end==lon_RTB)
    {
        /*
        doNavigation = false;
        doGuidance = false;
        semua hasil perhitungan navigasi = 0;
        semua hasil perhitungan guidance = 0;
        */
    } else
    {
        //doNavigation = true;
...
void SendStatusTS(int ports){
	...
	sprintf(PingReply,"#BB> ack PING %d%d%d%d%d%d\r\n",DateTime[0],
...
void FakeCoordinate(char* coordtxt, int line){
    int i = 0;
    string str;
    ifstream data(coordtxt);
    cout << "\nsetting is: " << endl;
    while(getline(data,str)){
        if(i == line){
            Longitude = stof(str.substr(0,str.find(" ")));
            Latitude = stof(str.substr(str.find(" "),str.find("\r\n")));
            cout << "Posisi sekarang : " << Longitude << " " << Latitude << endl;
            break;
        }
        i += 1;
    }
