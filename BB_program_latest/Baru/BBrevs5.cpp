...
inline void Logger2( string logMsg );
inline void Logger3( string logMsg );
// ------------------------------------------------------------------------
...
struct time{
	...
	struct timespec stopwatch1Start, stopwatch1Finish; // 
	struct timespec stopwatch2Start, stopwatch2Finish; // 
	struct timespec stopwatch3Start, stopwatch3Finish; // 
	double stopwatch1	= 0.0;
	double stopwatch2	= 0.0;
	double stopwatch3	= 0.0;
...
// Serial Setting ---------------------------------------------------------
    ...
    // Port TS
    TS.loc = "/dev/ttyS2";
    TS.port = openport(TS.loc, Baud_Sensor);
    if(!TS.isSet()) cout << TS.loc << "Open Error" << endl;
/*
    //Program  uji Altimeter menggunakan port USB0
    USB.loc ="/dev/ttyUSB0";
    USB.port =openport(USB.loc, Baud_Sensor);
...
// Loop Goes Here ---------------------------------------------------------
	...
	//start stopwatch3start
	clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch3Start);
    while(1){
		
		//start stopwatch1
		clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch1Start);
        ...
        if(Flag.BeagleMatlab == 0){
            ...
            if((Times.valSampling != 0 || Flag.BeagleActive) && (Flag.Sampling || Flag.BeagleActive)){
				//matiin ISR
				//set flagsampling ke 0
                Flag.Sampling = 0;
				alarm(Times.valSampling);
				
				clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch3Finish);
				// hitung selisih stopwatch3start - stopwatch3stop
				Times.stopwatch3 = (Times.stopwatch3Finish.tv_sec - Times.stopwatch3Start.tv_sec);
				Times.stopwatch3 += (Times.stopwatch3Finish.tv_nsec - Times.stopwatch3Start.tv_nsec) / 1000000000.0;
				//log data stopwatch 3
				// string stopwatch = to_string(Times.stopwatch3);
				// Logger3(to_string(Times.stopwatch3));
				clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch3Start);
				
				
				
				//start stopwatch2
				clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch2Start);
			...
			//string Alti_Data = ReadSerial(USB.port,"$PRVAT",1);
			//cout << Alti_Data<<"\n";
                    ...
                    if(Flag.State == 1){
                        ...
                        if(Flag.State > 1 || Flag.BeagleActive){
                            ...
                            if(Flag.State > 2 || Flag.BeagleActive){
                                // From IMU
                                dataSend.push_back(Sensors.valIMU.Omega_Pitch);
                                dataSend.push_back(Sensors.valIMU.Omega_Heading);
                                dataSend.push_back(Sensors.valIMU.Omega_Roll);
						...
						//stop stopwatch2
						clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch2Finish);
						Times.stopwatch2 = (Times.stopwatch2Finish.tv_sec - Times.stopwatch2Start.tv_sec);
						Times.stopwatch2 += (Times.stopwatch2Finish.tv_nsec - Times.stopwatch2Start.tv_nsec) / 1000000000.0;
						//log data stopwatch 2
						// string stopwatch = to_string(Times.stopwatch2);
						// Logger2(to_string(Times.stopwatch2));
            ...
            // if (Times.valSampling != 0 && Flag.State > 1){
				
                // if(Times.Sampling < (Times.valSampling)){
                    // Flag.Sampling = 0;

                    // usleep(50000);
                    // clock_gettime(CLOCK_MONOTONIC, &Times.finish);
                    // Times.times = (Times.finish.tv_sec - Times.start.tv_sec);
                    // Times.times += (Times.finish.tv_nsec - Times.start.tv_nsec) / 1000000000.0;
                    // Times.ElapsedTime += Times.times;
                    // printf ( "\nElapsed time in a loop\t: %f sec \r\n", times ); 
                    // printf ( "Elapsed time overall\t: %f sec \r\n", ElapsedTime ); 

                    // Times.Sampling += Times.times;
                    // cout << "\nSampling time\t: " << Times.valSampling << endl;
                    // cout << "Elapsed time\t: " << Times.Sampling << endl;
                    // cout << "Time Left\t: " << Times.valSampling-Times.Sampling << " sec" << endl;
                // }
                // else{
                    // Flag.Sampling = 1;
                    // Times.Sampling = 0;
		...
		if(Flag.State > 1){
			//stop stopwatch1
			clock_gettime(CLOCK_MONOTONIC, &Times.stopwatch1Finish);
			Times.stopwatch1 = (Times.stopwatch1Finish.tv_sec - Times.stopwatch1Start.tv_sec);
			Times.stopwatch1 += (Times.stopwatch1Finish.tv_nsec - Times.stopwatch1Start.tv_nsec) / 1000000000.0;
			//log data
			// string stopwatch = to_string(Times.stopwatch1);
			// Logger1(to_string(Times.stopwatch1));				
		}
    }
}
...
inline void Logger1( string logMsg ){
	// cout << "Creating stopwatch 1 log" << endl;
    string filePath = "/home/ubuntu/Baru/9600/1000ms/stopwatch1.txt";
...
inline void Logger2( string logMsg ){
	// cout << "Creating stopwatch 2 log" << endl;
    string filePath = "/home/ubuntu/Baru/9600/1000ms/stopwatch2.txt";
    // string now = getCurrentDateTime("now");
    ofstream ofs(filePath.c_str(), std::ios_base::out | std::ios_base::app );
    ofs << logMsg << '\n';
    ofs.close();
