...
//Program Debug definition
 //Set 0 Jika tidak dikoneksikan ke Maxon, 1 jika dikoneksikan. Program potensi Exit jika tidak berhasil mendeteksi Maxon
...
#define BeagleActive 1 //Set 1 jika rutin superloop(baca sensor, navigasi, guidance, dan send string ke TS) langsung jalan tanpa $#O1 dari command BB
...
int main(int argc, const char *argv[])
{
	...
	//gettimeofday(&start, NULL);
		...
		if (program_running == 0 || BeagleActive == 1)
		{
			//printf("Hei\n");
			//Trigger DVL
			TriggerDVL();
		...
		//Tes file datalog
			FILE *fptr;
			fptr = fopen("exampleAaja.txt","a");
			if(fptr == NULL)
			{
				write(fd,"File C failed\r\n",15);
				printf("File C failed\r\n");             
			}
			else
			{
				fprintf(fptr,"Hello\n");
			...
			ofstream myfileA;//,myfileB,myfileC,myfileD;
			myfileA.open ("exampleA++.txt", ios::out | ios::app);
			if(myfileA.fail())
			{
				write(fd,"File A failed\r\n",15);
				printf("File A failed\r\n");
			}
			myfileA << "Say Hello\n";
			myfileA.close();
			
			myfileB.open ("exampleB.txt", ios::out | ios::app, 1);
			myfileB << "Writing this to a file.\n";
			myfileB.close();
			myfileC.open ("exampleC.txt", ios::out | ios::app, 2);
			myfileC << "Writing this to a file.\n";
			myfileC.close();
			myfileD.open ("exampleD.txt", ios::out | ios::app, 4);
			myfileD << "Writing this to a file.\n";
			myfileD.close();
			printf("Done Log test\n");
			*/
		sleep(1);
		usleep(100000);
		}
