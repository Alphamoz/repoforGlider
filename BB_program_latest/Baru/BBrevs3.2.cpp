...
/*
    ...
    {
        USB.loc = "/dev/ttyIMU";
	cout << USB.loc << "Open sudah di open" << endl;
                    ...
                    else
                    {
                        ...
                        if (Flag.State > 1 || Flag.BeagleActive)
                        {
                            ...
                            if (Flag.State > 2 || Flag.BeagleActive)
                            {
                                ...
                                dataSend.push_back(Sensors.valIMU.posCov);
				// From DVL
                                ...
                                cout << "Status_GPS :" << Sensors.valIMU.Status <<" Position_covariance :"<< Sensors.valIMU.posCov << " Latutude :" << Sensors.valIMU.Latitude << " Longitude :" << Sensors.valIMU.Longitude << endl;
...
void getDVLdata()
{
    while (true)
    {
        ...
        //      << endl;
        //cout<<"Mulai function DVL data"<<endl;
        string gagal = "gagal";
        string time_out = "timeout";
        SerWrite(DVL.port, "#&!LQNQ.COMD2828#&!LQNQ.COMD5959 0\r\n");
        //cout<<"sudah serial Write"<<endl;
...
void ParseDVL(string Parse_DVL){
    ...
    iss >> header >> hex_value 
    >> b1 >> b2 >> b3 >> b4
    >> alti1 >> alti2 >> alti3 >> alti4
    >> velo_rad1 >> velo_rad2 >> velo_rad3 >> velo_rad4
    >> wvelo_rad1 >> wvelo_rad2 >> wvelo_rad3 >> wvelo_rad4
    >> wvelo_credit1 >> wvelo_credit2 >> wvelo_credit3 >> wvelo_credit4 
    >> velo_instrument1 >> velo_instrument2 >> velo_instrument3 >> flag_velo_instrument
    >> velo_earth1 >> velo_earth2 >> velo_earth3 >> flag_velo_earth
    >> water_velo_instrument1 >> water_velo_instrument2 >> water_velo_instrument3 >> flag_water_velo_instrument 
    >> water_velo_earth1 >> water_velo_earth2 >> water_velo_earth3 >> flag_water_velo_earth 
    >> rph1 >> rph2 >> rph3 
    >> alti >> temperature >> pressure >> salinity 
    >> sound_speed >> checksum;
