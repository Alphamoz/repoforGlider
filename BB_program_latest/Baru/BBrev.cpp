...
// Program Debug Definition ==============================================
#define IMUActive 0 //Set 0 Jika tidak dikoneksikan ke IMU via USB, 1 jika dikoneksikan. Program Exit jika tidak mendeteksi IMU
#define BeagleActive 0 //Set 1 jika rutin superloop(baca sensor, navigasi, guidance, dan send string ke TS) langsung jalan tanpa $#O1 dari command BB
#define SurfaceMode 0 //Set 1 jika navigasi deadreckoning tidak ingin diaktifkan dan hanya bergantung data GPS IMU(Misal pada mode surfacing saja)
#define SurfaceDepth 3 //Threshold(dalam meter) menentukan kapan hanya menggunakan data GPS(Perlu dikalibrasi). Asumsi permukaan, Jika Sounder_depth(Dari AltiMeter) > SurfaceDepth
#define DebugGuidance 0 //Set 1 untuk printf di terminal BB parameter2 Guidance untuk keperluan Debug
...
// All function declared here! ============================================
    ...
    int openport(char* portname, auto baudrate);
    string ReadSerial(int ports, string head, int line);
    int Com_Mainboard(int _fd, string command);
    void Navigasi(void);
...
// Com_Mainboard variables ================================================
    const string TS_BB_Deactivation  = "$#00";
    const string TS_BB_Activation    = "$#01";
    const string TS_Nav_Reset        = "$#02";
    const string TS_BB_Deactivation2 = "$#09";

    const string TS_Home_Setting     = "$#G1";
    const string TS_Target_Setting   = "$#G2";
    const string TS_Return_Setting   = "$#G3";

    const string TS_Scenario_Zero    = "$#S0";
    const string TS_Scenario_One     = "$#S1";

    const string TS_Relay_Off        = "$#R0";
    const string TS_Relay_On         = "$#R1";
    const string TS_Relay_Restart    = "$#R3";
...
// Initialize GPIO --------------------------------------------------------
    ...
    while(1){
        ...
        string TS_Command = ReadSerial(sport,"TS",1);
        cout << TS_Command << endl;
        Com_Mainboard(sport,TS_Command);

        clock_t start = clock();

        if (program_running == 1 || BeagleActive == 1){
            ...
            while(MiniCT_read_attempt < sensor_limit_attempt){
                cout << "Reading MiniCT" << endl;
                string MiniCT_Data = ReadSerial(sport1,"T=\t",1);
            ...
            while(Alti_read_attempt < sensor_limit_attempt){
                ...
                write(sport2, "S\r\n",3);
                // tcdrain(sport2);
                string Alti_Data = ReadSerial(sport2,"$PRVAT",1);
                cout << Alti_Data << endl;
                Alti_read_attempt += 1;
            ...
            while(DVL_read_attempt < sensor_limit_attempt){
                ...
                TriggerDVL();
                string DVL_Data = ReadSerial(sport3,"$#NQ.RES",2);
                cout << DVL_Data << endl;
                DVL_read_attempt += 1;
            ...
            Navigasi();
            if(DebugGuidance)printf("Current Heading =\t%f\n",Heading);
            GuidanceLOS();
...
int openport(char* portname, auto baudrate){
    ...
    newtio.c_cflag = (newtio.c_cflag & ~CSIZE) | CS8 | baudrate; // 8 databits
...
string ReadSerial(int ports, string head, int line){
    ...
    if(status == -1){
        ...
        while(j < line){
            int  bytes_read = 0;
            char * read_buffer = new char[254];                
            bytes_read = read(ports,read_buffer,254);
...
int Com_Mainboard(int _fd, string command){
    ...
    if (command.find(TS_BB_Activation) != std::string::npos){
        valid = 1;
        program_running = 1;
    ...
    if (command.find(TS_BB_Deactivation) != std::string::npos){
        valid = 1;
        program_running = 0;
        printf("Data received - DeActivation\n");
        write(_fd,"Beagle Deactivated/r/n",20);
    } else
    if (command.find(TS_BB_Deactivation2) != std::string::npos){
        closed_once = 1;
        valid = 1;
        program_running = 0;
    ...
    if (command.find(TS_Nav_Reset) != std::string::npos){
        valid = 1;
        program_running = 1;
        program_running_firsttime_in_the_water=1;
        resetNavigasi = 1;
        printf("Data received - Navigasi reset\n");
        write(_fd,"Navigation reset/r/n",18);
        tcdrain(_fd);
    }

    else
    if (command.find(TS_Home_Setting) != std::string::npos){
        guidance_setting = 1;
        printf("Receiving Home Setting\n");
        write(_fd,"$#G1 OK/r/n",9);
        tcdrain(_fd);
    } else
    if (command.find(TS_Target_Setting) != std::string::npos){
        guidance_setting = 2;
        printf("Receiving Target Setting\n");
        write(_fd,"$#G2 OK/r/n",9);
        tcdrain(_fd);
    } else
    if (command.find(TS_Return_Setting) != std::string::npos){
        guidance_setting = 3;
    ...
    if (command.find(TS_Scenario_One) != std::string::npos){
        // skenario = 1;
        // State_Scenario_1=1;
    ...
    if (command.find(TS_Scenario_Zero) != std::string::npos){
        // skenario = 0;
        // State_Scenario_1=0;
        printf("Receiving Scenario Setting\n");
    ...
    if (command.find(TS_Relay_On) != std::string::npos){
        Relay_On();
        printf("Relay ON\n");
        write(_fd,"Relay ON\r\n",10);
        tcdrain(_fd);
    } else
    if (command.find(TS_Relay_Off) != std::string::npos){
        Relay_Off();
        printf("Relay OFF\n");
        write(_fd,"Relay OFF\r\n",11); 
        tcdrain(_fd);
    } else
    if (command.find(TS_Relay_Restart) != std::string::npos){
...
void GuidanceLOS(void){
    ...
    if(distancefromtarget<=acceptance_radius && !guidance_finish){
        ...
        if(guidance_wp_target==waypoint_number){
            guidance_wp_target=waypoint_number;
            guidance_finish=1;
            //guidance_wp_target=guidance_wp_target-1;
        }
        else{
    ...
    if(guidance_finish){
        ...
        WpXnow = wpX[guidance_wp_target]; // wpX[1]; //Debug 4Des
        WpYnow = wpY[guidance_wp_target]; //wpY[1];//Debug 4Des
...
void SendTS(int ports)
{
    ...
    */
    ...
    stream_z    << fixed << setprecision(10) << Sounder_depth; //Z_Complement //Depth???? 1. Alti Depth 2. Navigasi Z topi?
    /*
    ...
    */
    ...
    stream_posx << fixed << setprecision(10) << posisiE_aug;
    stream_posy << fixed << setprecision(10) << posisiN_aug;
    stream_alti << fixed << setprecision(10) << altitudeMin_F;//Sounder_depth;
