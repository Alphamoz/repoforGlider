...
int main(){
    ...
    int fd = open("/sys/class/gpio/gpio26/value", O_RDWR | O_CLOEXEC ); // Relay$
    if( fd < 0 ) {
		...
		//usleep(200000);
                if(Flag.State > 0 || Flag.BeagleActive){
                    ...
                    if(Flag.IMUActive){
                        ...
                        if (smoot_IMU == 20){
			    now_sum_IMU_x = sum_IMU_x/20;
			    now_sum_IMU_y = sum_IMU_y/20;
			    now_sum_IMU_z = sum_IMU_z/20;
			    sum_IMU_x = 0;
			    sum_IMU_y = 0;
			    sum_IMU_z = 0;
			    smoot_IMU = 0;
...
//   double udot_ned = Sensors.valIMU.accel_X*(cos(Omega_Pitch_rad)*cos(Omega_Heading_rad))+
//		     Sensors.valIMU.accel_Y*(cos(Omega_Pitch_rad)*sin(Omega_Heading_rad))+
//                     Sensors.valIMU.accel_Z*(-1*sin(Omega_Pitch_rad));

//   double vdot_ned = Sensors.valIMU.accel_X*((sin(Omega_Roll_rad)*sin(Omega_Pitch_rad)*cos(Omega_Heading_rad))-(cos(Omega_Roll_rad)*sin(Omega_Heading_rad)))+
//                     Sensors.valIMU.accel_Y*(sin(Omega_Roll_rad)*sin(Omega_Pitch_rad)*sin(Omega_Heading_rad)+cos(Omega_Roll_rad)*cos(Omega_Heading_rad))+
//                     Sensors.valIMU.accel_Z*(sin(Omega_Roll_rad)*cos(Omega_Pitch_rad));

//   double wdot_ned = Sensors.valIMU.accel_X*(cos(Omega_Roll_rad)*sin(Omega_Pitch_rad)*cos(Omega_Heading_rad)+sin(Omega_Roll_rad)*sin(Omega_Heading_rad))+
//                     Sensors.valIMU.accel_Y*((cos(Omega_Roll_rad)*sin(Omega_Pitch_rad)*sin(Omega_Heading_rad))-(sin(Omega_Roll_rad)*cos(Omega_Heading_rad)))+
//                     Sensors.valIMU.accel_Z*(cos(Omega_Roll_rad)*cos(Omega_Pitch_rad))+g;
   printf("\n");
...
void LogData(){
    ...
    if (Log.newFile=1)
    {
        ...
        sprintf(datatulis,"accX, accY, accZ, Roll, Pitch, Heading, aIMU_N, aIMU_E, aIMU_D, longitude, latitude \n");
    ...
    Sensors.valIMU.accel_X, Sensors.valIMU.accel_Y, Sensors.valIMU.accel_Z, Sensors.valIMU.Roll, Sensors.valIMU.Pitch, Sensors.valIMU.Heading, aIMU_N, aIMU_E, aIMU_D,Sensors.valIMU.Longitude, Sensors.valIMU.Latitude
