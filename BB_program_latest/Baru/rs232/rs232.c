...
int OpenComport(char *comport, int baudrate)
{
  ...
  // new_port_settings.c_lflag |= ICANON; //Enable canonical mode
...
int OpenComport2(char *comport, int baudrate)
{
  int baudr;
  ...
  Cport2 = open(comport, O_RDWR | O_NOCTTY | O_NDELAY);
  if(Cport2==-1)
  {
    perror("unable to open comport ");
    return(1);
  ...
  error = tcgetattr(Cport2, &old_port_settings);
  if(error==-1)
  {
    close(Cport2);
    perror("unable to read portsettings ");
  ...
  new_port_settings.c_cc[VMIN] = 0;      /* block untill n bytes are received */
  new_port_settings.c_cc[VTIME] = 0;     /* block untill a timer expires (n * 100 mSec.) */
  error = tcsetattr(Cport2, TCSANOW, &new_port_settings);
  if(error==-1)
  {
    close(Cport2);
    perror("unable to adjust portsettings ");
...
int PollComport(unsigned char *buf, int size, int Cport_local)
{
  int n;
...
int SendAllData(double data1, double data2, double data3, double data4, double data5, double data6)
{
  ...
  sprintf(str,"%f,",data1);
  n = write(Cport2, &str, 8);
  sprintf(str,",");
  n = write(Cport2, &str, 1);
...
/* "warning: dereferencing type-punned pointer will break strict aliasing rules" */
  ...
  ReadFile(Cport_local, buf, size, (LPDWORD)((void *)&n), NULL);
...
int OpenComport2(char *comport, int baudrate)
{
  ...
  switch(baudrate)
  {
    case     110 : strcpy(baudr2, "baud=110 data=8 parity=N stop=1");
                   break;
    case     300 : strcpy(baudr2, "baud=300 data=8 parity=N stop=1");
                   break;
    case     600 : strcpy(baudr2, "baud=600 data=8 parity=N stop=1");
                   break;
    case    1200 : strcpy(baudr2, "baud=1200 data=8 parity=N stop=1");
                   break;
    case    2400 : strcpy(baudr2, "baud=2400 data=8 parity=N stop=1");
                   break;
    case    4800 : strcpy(baudr2, "baud=4800 data=8 parity=N stop=1");
                   break;
    case    9600 : strcpy(baudr2, "baud=9600 data=8 parity=N stop=1");
                   break;
    case   19200 : strcpy(baudr2, "baud=19200 data=8 parity=N stop=1");
                   break;
    case   38400 : strcpy(baudr2, "baud=38400 data=8 parity=N stop=1");
                   break;
    case   57600 : strcpy(baudr2, "baud=57600 data=8 parity=N stop=1");
                   break;
    case  115200 : strcpy(baudr2, "baud=115200 data=8 parity=N stop=1");
                   break;
    case  128000 : strcpy(baudr2, "baud=128000 data=8 parity=N stop=1");
                   break;
	case  230400 : strcpy(baudr2, "baud=230400 data=8 parity=N stop=1");
                   break;
    case  256000 : strcpy(baudr2, "baud=256000 data=8 parity=N stop=1");
                   break;
	case  460800 : strcpy(baudr2, "baud=460800 data=8 parity=N stop=1");
                   break;
	case  500000 : strcpy(baudr2, "baud=500000 data=8 parity=N stop=1");
                   break;
	case  921600 : strcpy(baudr2, "baud=921600 data=8 parity=N stop=1");
                   break;
	case 1000000 : strcpy(baudr2, "baud=1000000 data=8 parity=N stop=1");
                   break;
    default      : printf("invalid baudrate\n");
                   return(1);
                   break;
  ...
  Cport2 = CreateFileA(comport_path,
                      GENERIC_READ|GENERIC_WRITE,
                      0,                          /* no share  */
                      NULL,                       /* no security */
                      OPEN_EXISTING,
  ...
  {
    printf("unable to open comport2\n");
    return(1);
  }
  ...
  if(!SetCommState(Cport2, &port_settings))
  {
    printf("unable to set comport cfg settings\n");
    CloseHandle(Cport2);
    return(1);
  ...
  if(!SetCommTimeouts(Cport2, &Cptimeouts))
  {
    printf("unable to set comport time-out settings\n");
    CloseHandle(Cport2);
    return(1);
...
int SendByte2(char * byte)
{
  ...
  WriteFile(Cport2, byte, 8, (LPDWORD)((void *)&n), NULL);
