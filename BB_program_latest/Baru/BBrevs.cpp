...
// Whlie loop here! ----------------------------------------------------
    ...
    while(1){
        ...
        if (Ping == 1 || SensorRead == 1 || ProcessRunning == 1 || BeagleActive == 1){
            ...
            Volt = ReadAnalog(0);
            cout << "Analog channel 0: " << Volt << endl;
        ...
        if (ProcessRunning == 1){
            if(program_running_firsttime_in_the_water==1){
                lng_gps = Longitude;
                lat_gps = Latitude;
                depth_gps = 0.0; // where does this value come from ?!!
                Pitch_offset = Pitch;
                wpX[0]=Longitude;
                wpY[0]=Latitude;
                wpX[1]= 107.614731;
...
void Com_Mainboard(int _fd, string command){
    ...
    if (command.find(TS_BB_PING) != std::string::npos){
        ...
        Ping = 1;
        command.erase(0,TS_BB_PING.length()+1);
        ParsePing(command);
    } else
    if (command.find(TS_BB_DATA) != std::string::npos){
        SerWrite(_fd,"#BB> ack DATA\r\n");
        command.erase(0,TS_BB_DATA.length()+1);
        command.erase(1,2);
        if(command == "0"){
            SensorRead = 0;
        }
    ...
    // if (command.find(TS_BB_START) != std::string::npos){
    //     ProcessRunning = 1;
    //     program_running_firsttime_in_the_water=1;
    //     printf("While Loop Program Start\r\n");
    //     SerWrite(_fd,"#BB> ack START\r\n");
    // } else
    // if (command.find(TS_BB_STOP) != std::string::npos){
    //     ProcessRunning = 0;
    //     printf("Param received from TS\r\n");
    //     SerWrite(_fd,"#BB> ack STOP\r\n");
    // } else
