...
double g = 9.8335;
double init_Latitude[]={0};     //inisial pertama dari lat
double init_Longitude[]={0};   //inisial pertama dari long 
int Cont = 0;
int curStatus ;
int lastStatus ;
double lastgpsnorth;
...
Navigation calculateNavigation(Navigation _nav, Sensor sensor)
{
	...
	if (Cont == 0)
	{
	    nav.latitude_init = nav.latitude;
    ...
    }else if (curStatus == -1)
    {
        ...
        nav.N_aug += velocity*cos(yaw_rad);
        nav.E_aug += velocity*sin(yaw_rad);
    }
