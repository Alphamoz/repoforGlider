...
PID CalculatePID(PID _temp, PIDconst constants, float setPoint, float input, bool AW = false)
{
	...
	if (temp.name == "heading")
	{
		if (fabs(temp.error) > 2 && fabs(temp.error) < 10)
		{
...
void PIDconst::Setup(float _KP, float _KI, float _KD, float _AW, float _samplingtime, float _satUpper, float _satLower, float _midVal)
{
	KP = _KP;
	KI = _KI;
	KD = _KD;
			...
			if (std::getline(is_line, key, '='))
			{
				...
				if (std::getline(is_line, value))
				{
					...
					else if (key == "saturation_lower")
					{
						...
						std::istringstream(value) >> std::fixed >> std::setprecision(6) >> midVal;
					}
				}
			}
		}
