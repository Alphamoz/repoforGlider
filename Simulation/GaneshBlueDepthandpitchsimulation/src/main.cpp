...
void readDataArduino()
{
	...
	dcout("Recieved Data: " << AR.rawdata);

	std::string statusText;
    double BE_now;
    int MM_now;
	std::stringstream dataStream(AR.rawdata);
    dataStream >> statusText >> BE_now >> MM_now;
	BE_input = BE_now;
	MM_input = MM_now;
...
void getDataLog()
{
	...
	if (firstDataLog)
	{
		sprintf(buffer, "DepthNow DesDepth pitchNow desPitch BE_input MM_input");
		Utils::write2LOG(&*glider.datalogPath.begin(), buffer, false);
		firstDataLog = false;
	}
	// write to log
	sprintf(buffer, "%.6f %.6f %.6f %.6f %d %d",
			depthNow,
			desDepth,
			pitchNow,
			desPitch,
...
// Simple pitch mechanism for simulation
void pitchMechanism()
{
...
// simple depth mechanism for simulation
void depthMechanism()
{
	// asumsi 400 ml normal.
	// jika bertambah volumenya
	// F akan positif, yaitu ke bawah.
	int BE_Now = BE_input;
	while (true)
	{
		/* code */
		if ((BE_input - BE_Now)>10)
		{
			BE_Now += 10;
		}
		else if ((BE_input - BE_Now)<-10)
		{
			BE_Now -= 10;
		}
		else{
...
void manualGliding()
{
	int Pinit = 1; 		// initialize
	int P0 = 10;		// slightly pitchdown
	int P1 = 10;		// pitchdown
	int P2 = 60;		// go down 3m 
	int P3 = 60;		// staystill 3m
	int P4 = 10;		// pitching up
	int P5 = 60;		// going up 1m
	int P6 = 70; 		// staying still 1m
	if (first)
	{
